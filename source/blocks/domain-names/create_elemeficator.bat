...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/domain-names%filename%.pug
  )> newFile.pug
  type domain-names.pug >> newFile.pug
  move /Y newFile.pug  domain-names.pug
  (
...
(
echo @import '%fileName%/domain-names%fileName%';
)>>domain-names.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=domain-names
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=domain-names%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/domain-names%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
