...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/socials%filename%.pug
  )> newFile.pug
  type socials.pug >> newFile.pug
  move /Y newFile.pug  socials.pug
  (
...
(
echo @import '%fileName%/socials%fileName%';
)>>socials.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=socials
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=socials%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/socials%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
