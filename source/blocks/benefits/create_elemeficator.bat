...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/benefits%filename%.pug
  )> newFile.pug
  type benefits.pug >> newFile.pug
  move /Y newFile.pug  benefits.pug
  (
...
(
echo @import '%fileName%/benefits%fileName%';
)>>benefits.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=benefits
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=benefits%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/benefits%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
