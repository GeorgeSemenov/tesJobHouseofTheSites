...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/hosting-prices%filename%.pug
  )> newFile.pug
  type hosting-prices.pug >> newFile.pug
  move /Y newFile.pug  hosting-prices.pug
  (
...
(
echo @import '%fileName%/hosting-prices%fileName%';
)>>hosting-prices.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=hosting-prices
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=hosting-prices%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/hosting-prices%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
