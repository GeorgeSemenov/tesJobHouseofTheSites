...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/text-container%filename%.pug
  )> newFile.pug
  type text-container.pug >> newFile.pug
  move /Y newFile.pug  text-container.pug
  (
...
(
echo @import '%fileName%/text-container%fileName%';
)>>text-container.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=text-container
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=text-container%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/text-container%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
