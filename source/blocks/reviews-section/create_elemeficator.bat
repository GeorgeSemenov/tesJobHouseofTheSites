set parentName=reviews-section
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/reviews-section%filename%.pug
  )> newFile.pug
  type reviews-section.pug >> newFile.pug
  move /Y newFile.pug  reviews-section.pug
  (
...
(
echo @import '%fileName%/reviews-section%fileName%';
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=reviews-section
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=reviews-section%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/reviews-section%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
