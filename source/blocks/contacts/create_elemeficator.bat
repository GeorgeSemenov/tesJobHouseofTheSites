...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/contacts%filename%.pug
  )> newFile.pug
  type contacts.pug >> newFile.pug
  move /Y newFile.pug  contacts.pug
  (
...
(
echo @import '%fileName%/contacts%fileName%';
)>>contacts.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=contacts
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=contacts%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/contacts%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
