set parentName=questions-and-answers
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/questions-and-answers%filename%.pug
  )> newFile.pug
  type questions-and-answers.pug >> newFile.pug
  move /Y newFile.pug  questions-and-answers.pug
  (
...
(
echo @import '%fileName%/questions-and-answers%fileName%';
)>>questions-and-answers.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=questions-and-answers
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=questions-and-answers%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/questions-and-answers%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
