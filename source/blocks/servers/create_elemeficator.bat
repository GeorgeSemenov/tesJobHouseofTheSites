...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/servers%filename%.pug
  )> newFile.pug
  type servers.pug >> newFile.pug
  move /Y newFile.pug  servers.pug
  (
...
(
echo @import '%fileName%/servers%fileName%';
)>>servers.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=servers
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=servers%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/servers%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
