...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/CMS-support%filename%.pug
  )> newFile.pug
  type CMS-support.pug >> newFile.pug
  move /Y newFile.pug  CMS-support.pug
  (
...
(
echo @import '%fileName%/CMS-support%fileName%';
)>>CMS-support.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=CMS-support
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=CMS-support%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/CMS-support%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
