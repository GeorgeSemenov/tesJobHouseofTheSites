...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/logotype%filename%.pug
  )> newFile.pug
  type logotype.pug >> newFile.pug
  move /Y newFile.pug  logotype.pug
  (
...
(
echo @import '%fileName%/logotype%fileName%';
)>>logotype.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=logotype
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=logotype%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/logotype%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
