...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/ssl-cert%filename%.pug
  )> newFile.pug
  type ssl-cert.pug >> newFile.pug
  move /Y newFile.pug  ssl-cert.pug
  (
...
(
echo @import '%fileName%/ssl-cert%fileName%';
)>>ssl-cert.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=ssl-cert
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=ssl-cert%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/ssl-cert%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
