...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/ways-to-pay%filename%.pug
  )> newFile.pug
  type ways-to-pay.pug >> newFile.pug
  move /Y newFile.pug  ways-to-pay.pug
  (
...
(
echo @import '%fileName%/ways-to-pay%fileName%';
)>>ways-to-pay.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=ways-to-pay
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=ways-to-pay%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/ways-to-pay%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
