...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/proposal-card%filename%.pug
  )> newFile.pug
  type proposal-card.pug >> newFile.pug
  move /Y newFile.pug  proposal-card.pug
  (
...
(
echo @import '%fileName%/proposal-card%fileName%';
)>>proposal-card.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=proposal-card
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=proposal-card%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/proposal-card%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
