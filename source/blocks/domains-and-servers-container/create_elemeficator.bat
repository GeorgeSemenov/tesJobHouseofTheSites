set parentName=domains-and-servers-container
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/domains-and-servers-container%filename%.pug
  )> newFile.pug
  type domains-and-servers-container.pug >> newFile.pug
  move /Y newFile.pug  domains-and-servers-container.pug
  (
...
(
echo @import '%fileName%/domains-and-servers-container%fileName%';
)>>domains-and-servers-container.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=domains-and-servers-container
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=domains-and-servers-container%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/domains-and-servers-container%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
