...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/faq-section%filename%.pug
  )> newFile.pug
  type faq-section.pug >> newFile.pug
  move /Y newFile.pug  faq-section.pug
  (
...
(
echo @import '%fileName%/faq-section%fileName%';
)>>faq-section.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=faq-section
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=faq-section%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/faq-section%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
