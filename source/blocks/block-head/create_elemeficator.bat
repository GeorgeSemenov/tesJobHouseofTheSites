...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/block-head%filename%.pug
  )> newFile.pug
  type block-head.pug >> newFile.pug
  move /Y newFile.pug  block-head.pug
  (
...
(
echo @import '%fileName%/block-head%fileName%';
)>>block-head.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=block-head
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=block-head%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/block-head%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
