set parentName=advantages-list
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/advantages-list%filename%.pug
  )> newFile.pug
  type advantages-list.pug >> newFile.pug
  move /Y newFile.pug  advantages-list.pug
  (
...
(
echo @import '%fileName%/advantages-list%fileName%';
)>>advantages-list.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=advantages-list
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=advantages-list%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/advantages-list%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
