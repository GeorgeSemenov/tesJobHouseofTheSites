...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/footer%filename%.pug
  )> newFile.pug
  type footer.pug >> newFile.pug
  move /Y newFile.pug  footer.pug
  (
...
(
echo @import '%fileName%/footer%fileName%';
)>>footer.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=footer
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=footer%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/footer%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
